function table_content = read_R_csv_faster(filename)
...
subsequent_rows = parse_multiple_rows(d((row_end_ind(1)+1):end), num_rows-1, sep);
% subsequent_rows = [];
% for i=2:num_rows
%     i
...
function subsequent_rows = parse_multiple_rows(txt, num_rows, sep)

quote_ind = find(txt=='"');
quote_start_ind = quote_ind(1:2:end);
quote_end_ind = quote_ind(2:2:end);
inside_indicator = logical(zeros(1,length(txt)));
for i=1:length(quote_start_ind)
    inside_indicator(quote_start_ind(i):quote_end_ind(i))=1;
end


sep_ind = find(txt==sep & inside_indicator==0);
txt(sep_ind) = char(1);
txt(txt==char(10)) = char(1);
if txt(end)==char(1)
    txt(end) = [];
end
tmp = regexp(txt,char(1),'split');
subsequent_rows = reshape(tmp,length(tmp)/num_rows,num_rows)';
