    ...
    methods
        ...
        function obj = Read10X(obj, folder_name)
            ...
            if fid==-1
                ...
                filenames_tmp = filenames_tmp(isInList(filenames_tmp,'barcodes.tsv'));
            ...
            if fid==-1
                filenames_tmp = getfilenames(folder_name);
                filenames_tmp = filenames_tmp(isInList(filenames_tmp,'genes.tsv'));
                fid = fopen(fullfile(folder_name,filenames_tmp{1}));
            end
            d = fread(fid,[1,inf],'char'); d = char(d); fclose(fid);
            d(d==char(13))=[];
            ...
            if fid==-1
                filenames_tmp = getfilenames(folder_name);
                filenames_tmp = filenames_tmp(isInList(filenames_tmp,'matrix.mtx'));
                fid = fopen(fullfile(folder_name,filenames_tmp{1}));
            end
            d = fread(fid,[1,inf],'char'); d = char(d); fclose(fid);
        ...
        function obj = ReadMatlab(obj, filename)
            ...
            obj.raw_gene_names = raw_gene_names;
            obj.raw_cell_names = raw_cell_names;
        end
