...
% NOTE: following block added on 5/29/2019
if size(selected_data_gene_cluster_mean,1)<3
    tmp = [];
    for i=1:max(cell_labels)
        for j=i+1:max(cell_labels)
            ...
            tmp = [tmp; [i,j, best_snr3]];
        end
    end
    tmp(isnan(tmp(:,3)),3) = 0;
    tmp(isinf(tmp(:,3)),3) = max(tmp(~isinf(tmp(:,3)),3));
    ...
    for i=1:size(edges,1)
        ...
        scores(i) = compute_snr_between_within(selected_data_gene_cluster_mean(:,ismember(cell_labels,component_1)),selected_data_gene_cluster_mean(:,ismember(cell_labels,component_2)));
    end
...
cluster_size(1)=[];
    ...
    % % NOTE: following block replaced by the subsequent one line on 5/29/2019
    % if size(selected_data_gene_cluster_mean,1)>=3
